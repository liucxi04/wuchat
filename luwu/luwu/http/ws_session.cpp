    ...
    namespace http {
        ...
        HttpRequest::ptr WSSession::handleShake() {
            ...
            do {
                req = recvRequest();
                if (!req) {
                    LUWU_LOG_INFO(g_logger) << "invalid http request";
                    break;
                }
                if (strcasecmp(req->getHeader("Upgrade").c_str(), "websocket")) {
                    LUWU_LOG_INFO(g_logger) << "http header Upgrade != websocket";
                    break;
                }
                if (strcasecmp(req->getHeader("Connection").c_str(), "Upgrade")) {
                    LUWU_LOG_INFO(g_logger) << "http header Connection != Upgrade";
                    break;
                }
                if (req->getHeaderAs<int>("Sec-webSocket-Version") != 13) {
                    LUWU_LOG_INFO(g_logger) << "http header Sec-webSocket-Version != 13";
                    break;
                }
                std::string key = req->getHeader("Sec-WebSocket-Key");
                if (key.empty()) {
                    LUWU_LOG_INFO(g_logger) << "http header Sec-WebSocket-Key = null";
                    break;
                }
                ...
                sendResponse(rsp);
                LUWU_LOG_DEBUG(g_logger) << *req;
                LUWU_LOG_DEBUG(g_logger) << *rsp;
                return req;
            } while (false);
            if (req) {
                LUWU_LOG_INFO(g_logger) << *req;
            }
            return nullptr;
        }
        ...
        WSFrameMessage::ptr WSRecvMessage(Stream *stream, bool client) {
            ...
            do {
                ...
                LUWU_LOG_DEBUG(g_logger) << "WSFrameHead " << ws_head.toString();

                if (ws_head.opcode == WSFrameHead::PING) {
                    LUWU_LOG_INFO(g_logger) << "PING";
                    if (WSPong(stream) <= 0) {
                        break;
                    }
                    ...
                    if (!client && !ws_head.mask) {
                        LUWU_LOG_INFO(g_logger) << "WSFrameHead mask != 1";
                        break;
                    }
                    uint64_t length = 0;
                    ...
                    if ((cur_len + length) >= g_websocket_message_max_size->getValue()) {
                        LUWU_LOG_WARN(g_logger) << "WSFrameMessage length > "
                    ...
                    if (ws_head.fin) {
                        LUWU_LOG_DEBUG(g_logger) << data;
                        return WSFrameMessage::ptr(new WSFrameMessage(opcode, std::move(data)));
                    }
                } else {
                    LUWU_LOG_DEBUG(g_logger) << "invalid opcode=" << ws_head.opcode;
                }
            } while (true);
            stream->close();
            return nullptr;
