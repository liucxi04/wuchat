 ...
 * @date 2021-09-25
 */
#include "luwu/http/http_parser.h"
#include <iostream>
...
void test_request(const char *str) {
    liucxi::http::HttpRequestParser parser;
    std::string tmp = str;
    ...
    if (parser.getError()) {
        ...
        liucxi::http::HttpRequest::ptr req = parser.getData();
        std::cout << req->toString() << std::endl;
    }
}

void test_response(const char *str) {
    liucxi::http::HttpResponseParser parser;
    std::string tmp = str;
    ...
    if (parser.getError()) {
        ...
        liucxi::http::HttpResponse::ptr rsp = parser.getData();
