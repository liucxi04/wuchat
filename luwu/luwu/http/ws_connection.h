...
namespace liucxi {
    namespace http {
        ...
        public:
            ...
            Create(const std::string &url, uint64_t timeout_ms, const std::map<std::string, std::string> &headers = {});

            static std::pair<HttpResult::ptr, WSConnection::ptr>
            Create(URI::ptr uri, uint64_t timeout_ms, const std::map<std::string, std::string> &headers = {});
